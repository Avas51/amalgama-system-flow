import { motion } from 'framer-motion';

type Mode = 'alpha' | 'beta' | 'gamma';

interface ModeSelectorProps {
  activeMode: Mode;
  onModeChange: (mode: Mode) => void;
}

const modes = [
  {
    id: 'alpha' as Mode,
    name: 'АЛЬФА',
    wakeTime: 'Подъём 4:00',
    subtitle: 'Идеальный',
    bgColor: 'bg-blue-500/10',
    borderColor: 'border-blue-400/30',
    icon: '⚡',
  },
  {
    id: 'beta' as Mode,
    name: 'БЕТА',
    wakeTime: 'Подъём 5:00–6:30',
    subtitle: 'Адаптивный',
    bgColor: 'bg-amber-500/10',
    borderColor: 'border-amber-400/30',
    icon: '🌊',
  },
  {
    id: 'gamma' as Mode,
    name: 'ГАММА',
    wakeTime: 'Подъём 7:00+',
    subtitle: 'Кризисный',
    bgColor: 'bg-red-500/10',
    borderColor: 'border-red-400/30',
    icon: '🔥',
  },
];

export default function ModeSelector({ activeMode, onModeChange }: ModeSelectorProps) {
  return (
    <div className="space-y-2">
      <h2 className="text-sm font-semibold text-primary-foreground/60 mb-2">Режим</h2>
      
      {/* Compact cards in a row */}
      <div className="grid grid-cols-3 gap-2">
        {modes.map((mode) => (
          <motion.button
            key={mode.id}
            onClick={() => onModeChange(mode.id)}
            whileTap={{ scale: 0.95 }}
            className={`relative p-3 rounded-xl transition-all duration-200 text-center ${
              activeMode === mode.id
                ? `${mode.bgColor} ${mode.borderColor} border`
                : 'bg-card/20 border border-primary-foreground/10 hover:bg-card/30'
            }`}
          >
            {/* Icon */}
            <motion.div
              animate={{ scale: activeMode === mode.id ? 1.1 : 1 }}
              className="text-xl mb-1"
            >
              {mode.icon}
            </motion.div>
            
            {/* Name - larger */}
            <h3 className={`text-base font-bold tracking-wide ${
              activeMode === mode.id ? 'text-accent' : 'text-primary-foreground/70'
            }`}>
              {mode.name}
            </h3>
            
            {/* Wake time + subtitle in one line */}
            <p className="text-xs text-primary-foreground/50 mt-0.5">
              {mode.wakeTime} • {mode.subtitle}
            </p>
            
            {/* Active indicator */}
            {activeMode === mode.id && (
              <motion.div
                layoutId="activeModeIndicator"
                className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 bg-accent rounded-full"
                transition={{ type: 'spring', stiffness: 300, damping: 30 }}
              />
            )}
          </motion.button>
        ))}
      </div>

      {/* Info - compact */}
      <p className="text-xs text-primary-foreground/40 text-center">
        💡 Выберите режим по времени пробуждения
      </p>
    </div>
  );
}
